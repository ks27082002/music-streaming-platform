...
const UploadModal = () => {
  ...
  const {register, handleSubmit, reset} = useForm<FieldValues>({//useForm hook used to handle forms in react
    defaultValues:{
      author:'',
      title:'',
      song:null,
  ...
  const onChange = (open:boolean) => {//this is handled by Dialog.Root
    if (!open){
      reset()
      uploadModal.onClose()
    }
